...
public:
    ...
    std::string
    toJSON(JSValue replacer = JS_UNDEFINED, JSValue space = JS_UNDEFINED)
    {
        assert(ctx);
        return (std::string) Value{ctx, JS_JSONStringify(ctx, v, replacer, space)};
    }
...
#ifdef TRACK_UNHANDLED_PROMISE_REJECTIONS
